...
class DynamicMemoryWeightsLossAttention(nn.Module):
    """concept attention"""
    def __init__(self, k, ch, which_conv, pool_size_per_cluster, num_k, feature_dim, warmup_total_iter=1000, cp_momentum=0.3, \
        ...
        self.phi_k = [self.which_conv(
            self.ch, self.feature_dim, kernel_size=1, padding=0, bias=False).cuda()]  # using list to prevent pytorch consider phi_k as a parameter to optimize
    ...
    @torch.no_grad()
    # Weight-Loss Dynamic (W-LD) update strategy
    def forward_update_pool(self, activation, cluster_num, momentum=None):
        ...
        for cluster in range(self.num_k):
            cluster_concept_pool = self.concept_pool.detach().clone()[:, cluster * self.pool_size_per_cluster: (cluster + 1) * self.pool_size_per_cluster].cuda()
            part_concept_pool = F.softmax(cluster_concept_pool, dim=-1)
            part_concept_pool = torch.sum(part_concept_pool, dim=0)
            index_1 = torch.topk(part_concept_pool, k=int(self.k), dim=-1, largest=False)[1]  #Find the minimum value
            ...
            if cluster_index.shape[0] == 0 :
                ...
                index_2 = torch.topk(cluster_updata, k=int(self.k), dim=-1, largest=True)[1]  #Find the maximum value
                cluster_concept_pool[:, index_1] = (1. - momentum) * cluster_concept_pool[:, index_1.cuda()] + momentum * cluster_activation[:, index_2.cuda()]
                q.append(cluster_concept_pool)
